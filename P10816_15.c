...
RBnode *pNIL;//=(RBnode *)malloc(sizeof(RBnode));
...
void preOrder(RBnode *root)
{
    ...
    printf("%d(color: %d, count: %d) ",root->data, root->color, root->count);
    preOrder(root->left);
    preOrder(root->right);
}
    ...
    if(P->color==RED && U->color==BLACK)
    {
        ...
        if(G_to_P != P_to_C)
        {
            ...
            if(G_to_P==LEFT) // P_to_C==RIGHT
            {
                ...
                rotation_result=left_rotation(P);
                G->left=rotation_result;
                rotation_result->parent=G;
            }
            else if(G_to_P==RIGHT) // P_to_C==LEFT
            {
                // right rotation on P
                printf("right rotation on %d\n", P->data);
                rotation_result=right_rotation(P);
                G->right=rotation_result;
                rotation_result->parent=G;
            }
            SWAP(P, C, temp_node);
        ...
        // attach
        printf("<attatch>\n");
        printf("attach to->data: %d, rotation_result->data: %d\n", (attatch_to->data),(rotation_result->data));
